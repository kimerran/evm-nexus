// GET /api/deployments/:id — deployment detail + live status (SPEC §8.5).
//
// Auth re-checked here. Scoped to the authenticated user — a foreign id reads as
// 404 (never leaks another user's deployment). Next 16 route `params` is async.
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    return jsonOk({ deployment: toDeploymentView(row) });
  } catch (err) {
